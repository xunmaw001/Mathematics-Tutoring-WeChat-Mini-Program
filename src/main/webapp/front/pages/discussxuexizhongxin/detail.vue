...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscussxuexizhongxinCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscussxuexizhongxinCleanType')
				this.mescroll.num = 1
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discussxuexizhongxin');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','discussxuexizhongxin');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('discussxuexizhongxin', this.id);
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscussxuexizhongxin', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussdiscussxuexizhongxin/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('discussxuexizhongxin', this.detail);
				this.$utils.msg('审核成功');
